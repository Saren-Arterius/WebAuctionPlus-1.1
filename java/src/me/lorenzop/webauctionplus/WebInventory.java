...
public class WebInventory {
    ...
    public WebInventory(final Player player) {
        ...
        final int numSlots = WebAuctionPlus.MinMax(WebAuctionPlus.settings.getInteger("Inventory Rows"), 1, 6) * 9;
        String invTitle = WebAuctionPlus.Lang.getString("mailbox_title");
        if (invTitle == null || invTitle.isEmpty()) {
            invTitle = "WebAuction+ MailBox";
        }
        chest = Bukkit.createInventory(null, numSlots, invTitle);
        loadInventory();
    ...
    public static void onInventoryOpen(final Player player) {
        ...
        synchronized (WebInventory.openInvs) {
            // lock inventory
            WebInventory.setLocked(playerName, true);
            ...
            WebInventory.openInvs.put(playerName, inventory);
        }
    ...
    public static void ForceCloseAll() {
        if (WebInventory.openInvs == null || WebInventory.openInvs.size() == 0) {
            return;
        }
        for (final String playerName: WebInventory.openInvs.keySet()) {
    ...
    // WebAuctionPlus.log.info("WA Query: isLocked");
    // st =
    // conn.prepareStatement("SELECT `Locked` FROM `"+WebAuctionPlus.dataQueries.dbPrefix()+"Players` "+
    ...
    protected void loadInventory() {
        ...
        try {
            ...
            st = conn.prepareStatement("SELECT `id`, `itemId`, `itemDamage`, `qty`, `enchantments`, `itemTitle` "
                    + "FROM `" + WebAuctionPlus.dataQueries.dbPrefix()
                    + "Items` WHERE `playerName` = ? ORDER BY `id` ASC LIMIT ?");
            st.setString(1, playerName);
            st.setInt(2, chest.getSize());
            rs = st.executeQuery();
            final ItemStack[] stacks = new ItemStack[chest.getSize()];
            int i = -1;
            while (rs.next()) {
                if (rs.getInt("qty") < 1) {
                    continue;
                ...
                stacks[i] = getSplitItemStack(rs.getInt("itemId"), rs.getShort("itemDamage"), rs.getInt("qty"),
                        rs.getString("enchantments"), rs.getString("itemTitle"));
                if (stacks[i] == null) {
                    tableRowIds.remove(i);
                }
            }
    ...
    // create/split item stack
    private ItemStack getSplitItemStack(final int itemId, final short itemDamage, final int qty, final String enchStr,
            final String itemTitle) {
        final Material mat = Material.matchMaterial(Integer.toString(itemId));
        if (mat == null) {
            (new NullPointerException("Unknown material id: " + Integer.toString(itemId))).printStackTrace();
            return null;
        }
        int tmpQty = qty;
        ...
        if (qty > maxSize) {
            ...
            while (tmpQty > maxSize) {
                try {
                    WebAuctionPlus.getLog().debug(
                            "WA Query: getSplitItemStack  qty:" + Integer.toString(tmpQty) + "  max:"
    ...
    protected void saveInventory() {
        ...
        for (int i = 0; i < chest.getSize(); i++) {
            ...
            final ItemStack stack = chest.getItem(i);

            // empty slot
            if (stack == null || WebInventory.getTypeId(stack) == 0) {

                // delete item
                if (tableRowIds.containsKey(i)) {
                    try {
                        WebAuctionPlus.getLog().debug("WA Query: saveInventory::delete slot " + Integer.toString(i));
                        st = conn.prepareStatement("DELETE FROM `" + WebAuctionPlus.dataQueries.dbPrefix()
