...
public class DataQueries extends MySQLConnPool {
    ...
    public DataQueries(String dbHost, int dbPort, String dbUser, String dbPass, String dbName, String dbPrefix) {
        super();
        this.dbHost = dbHost;
        this.dbPort = dbPort;
        this.dbUser = dbUser;
    ...
    public Auction getAuction(int auctionId) {
        ...
        try {
            ...
            st = conn
                    .prepareStatement("SELECT `playerName`, `itemId`, `itemDamage`, `qty`, `enchantments`, `itemTitle`, "
                            + "`price`, `allowBids`, `currentBid`, `currentWinner` FROM `"
                            + dbPrefix
                            + "Auctions` WHERE `id` = ? LIMIT 1");
            ...
            if (rs.next()) {
                ...
                WebItemMeta.encodeItem(stack, null);
    ...
    public void createShoutSign(World world, int radius, int x, int y, int z) {
        ...
        try {
            if (isDebug()) {
                log.info("WA Query: createShoutSign " + Integer.toString(radius) + " " + Integer.toString(x) + ","
    ...
    public void removeShoutSign(Location location) {
        ...
        try {
            if (isDebug()) {
                log.info("WA Query: removeShoutSign " + location.toString());
            }
    ...
    public Map<Location, Integer> getShoutSignLocations() {
        final Map<Location, Integer> signLocations = new HashMap<Location, Integer>();
        final Connection conn = getConnection();
        PreparedStatement st = null;
        ResultSet rs = null;
        try {
    ...
    public void createRecentSign(World world, int offset, int x, int y, int z) {
        ...
        final ResultSet rs = null;
        try {
            log.debug("WA Query: createRecentSign " + world.getName() + " " + Integer.toString(offset) + " "
    ...
    public AuctionPlayer getPlayer(final String playerName) {
        ...
        try {
            ...
            st = conn.prepareStatement("SELECT `id`,`playerName`,`money`,`Permissions` " + "FROM `" + dbPrefix
                    + "Players` WHERE `playerName` = ? LIMIT 1");
    ...
    public void createPlayer(AuctionPlayer waPlayer, String pass) {
        ...
        final ResultSet rs = null;
        try {
            log.debug("WA Query: createPlayer " + waPlayer.getPlayerName() + " with perms: "
                    + waPlayer.getPermsString());
    ...
    public void updatePlayerPassword(String player, String newPass) {
        ...
        try {
            ...
            st = conn.prepareStatement("UPDATE `" + dbPrefix
                    + "Players` SET `password` = ? WHERE `playerName` = ? LIMIT 1");
    ...
    public void updatePlayerPermissions(AuctionPlayer waPlayer, boolean canBuy, boolean canSell, boolean isAdmin) {
        ...
        if (waPlayer.comparePerms(canBuy, canSell, isAdmin)) {
            return;
        }
        final Connection conn = getConnection();
        PreparedStatement st = null;
        ...
        try {
            ...
            log.debug("WA Query: updatePlayerPermissions " + waPlayer.getPlayerName() + " with perms: "
                    + waPlayer.getPermsString());
