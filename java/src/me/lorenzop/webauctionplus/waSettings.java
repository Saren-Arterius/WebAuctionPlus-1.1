...
public class waSettings {
    ...
    public synchronized void LoadSettings() {
        ...
        try {
            ...
            st = conn.prepareStatement("SELECT `name`, `value` FROM `" + WebAuctionPlus.dataQueries.dbPrefix()
                    + "Settings`");
            rs = st.executeQuery();
            while (rs.next()) {
    ...
    private void addDefault(String name, String value) {
        if (!settingsMap.containsKey(name)) {
            ...
            try {
                st = conn.prepareStatement("INSERT INTO `" + WebAuctionPlus.dataQueries.dbPrefix()
    ...
    private void addDefault(String name, boolean value) {
        if (value) {
            addDefault(name, "true");
        } else {
            addDefault(name, "false");
        }
    }
    ...
    private void updateSettingsTable() {
        if (settingsMap.containsKey("jquery ui pack")) {
            ...
            WebAuctionPlus.dataQueries.executeRawSQL("UPDATE `" + WebAuctionPlus.dataQueries.dbPrefix()
                    + "Settings` SET `name` = 'jQuery UI Pack' WHERE `name` = 'jquery ui pack' LIMIT 1");
    ...
    public synchronized void setString(String name, String value) {
        ...
        try {
            st = conn.prepareStatement("UPDATE `" + WebAuctionPlus.dataQueries.dbPrefix()
                    + "Settings` SET `value` = ? WHERE `name` = ? LIMIT 1");
