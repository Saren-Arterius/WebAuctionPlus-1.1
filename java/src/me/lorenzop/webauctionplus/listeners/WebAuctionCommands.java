...
public class WebAuctionCommands implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
            ...
            if (args[0].equalsIgnoreCase("reload")) {
                if (sender instanceof Player) {
                    if (!sender.hasPermission("wa.reload")) {
                        ((Player) sender).sendMessage(WebAuctionPlus.chatPrefix
                                + WebAuctionPlus.Lang.getString("no_permission"));
                        return true;
                    }
                }
                if (sender instanceof Player) {
                ...
                if (WebAuctionPlus.isOk()) {
                    if (sender instanceof Player) {
                        sender.sendMessage(WebAuctionPlus.chatPrefix
                                + WebAuctionPlus.Lang.getString("finished_reloading"));
                    }
            ...
            if (args[0].equalsIgnoreCase("version")) {
                if (sender instanceof Player) {
                    ...
                    if (WebAuctionPlus.newVersionAvailable && sender.hasPermission("wa.webadmin")) {
                        sender.sendMessage(WebAuctionPlus.chatPrefix + "A new version is available! "
        ...
        if (params == 2 || params == 3) {
            // wa password
            if (args[0].equalsIgnoreCase("password") || args[0].equalsIgnoreCase("pass")
                    || args[0].equalsIgnoreCase("pw")) {
                String pass = "";
                // is player
                final boolean isPlayer = (sender instanceof Player);
                if (isPlayer) {
                    if (params != 2 || args[1].isEmpty()) {
                        return false;
                    }
                    ...
                    if (!Bukkit.getOfflinePlayer(player).hasPlayedBefore()) {
                        ...
                        sender.sendMessage(WebAuctionPlus.chatPrefix
                                + "Note: if you really need to, you can add a player to the database, just md5 the password.");
                        return true;
                    }
                ...
                if (waPlayer == null) {
                    ...
                    waPlayer.setPerms(sender.hasPermission("wa.canbuy") && isPlayer, sender.hasPermission("wa.cansell")
                            && isPlayer, sender.hasPermission("wa.webadmin") && isPlayer);
                    ...
                    WebAuctionPlus.getLog().info(
                            WebAuctionPlus.Lang.getString("account_created") + " " + player + " with perms: "
                    ...
                    if (sender instanceof Player) {
                        sender.sendMessage(WebAuctionPlus.chatPrefix
                                + WebAuctionPlus.Lang.getString("password_changed"));
                    }
