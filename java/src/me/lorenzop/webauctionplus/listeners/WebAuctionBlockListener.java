...
public class WebAuctionBlockListener implements Listener {
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        ...
        if (WebAuctionBlockListener.getTypeId(block) == 63 || WebAuctionBlockListener.getTypeId(block) == 68) {
            final Sign thisSign = (Sign) block.getState();
    ...
    public void onSignChange(SignChangeEvent event) {
        ...
        if (!lines[0].equalsIgnoreCase("[WebAuction]") && !lines[0].equalsIgnoreCase("[WebAuction+]")
        ...
        if (lines[1].equalsIgnoreCase("Shout")) {
            if (!p.hasPermission("wa.create.sign.shout")) {
                WebAuctionBlockListener.NoPermission(event);
                return;
            }
        ...
        if (lines[1].equalsIgnoreCase("Recent")) {
            ...
            // Auction offsetAuction =
            // WebAuctionPlus.dataQueries.getAuctionForOffset(offset - 1);
            // ItemStack stack = offsetAuction.getItemStack();
            // int qty = stack.getAmount();
            // String formattedPrice =
            // plugin.economy.format(offsetAuction.getPrice());
        ...
        WebAuctionBlockListener.CancelEvent(event);
    }
