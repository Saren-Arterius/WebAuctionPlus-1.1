...
public class RecentSignTask implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                final String strPrice = WebAuctionPlus.FormatPrice(rs.getFloat("price"));
                final String strQtyPrice = Integer.toString(qty) + "x " + strPrice;
                String strSeller = rs.getString("playerName");
                if (strSeller == null || strSeller.isEmpty()) {
                    strSeller = "N/A       ";
                    // String strType = "";
    ...
    private void UpdateRecentSigns(int offset, String[] lines) {
        ...
        for (final Entry<Location, Integer> entry: plugin.recentSigns.entrySet()) {
            try {
    ...
    public Block getBlockSafely(final Location loc) {
        ...
        } catch (final InterruptedException ignore) {} catch (final ExecutionException ignore) {}
        return null;
    }
    ...
    // if(text.length() < size) return text+StringUtils.repeat(" ",
    // size-text.length());
    // return text;
    // }
